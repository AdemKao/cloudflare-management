...
The executable command is `cfm` (with `cloudflare-management` as an alias).

## Why

This is intended for a freelance/multi-client workflow where one Mac may need to connect several independent Cloudflare accounts without repeatedly running `cloudflared tunnel login` or mixing account credentials.

Each client keeps its own remotely-managed Tunnel in its own Cloudflare account. This CLI only stores that Tunnel's connector token locally and starts/stops the matching `cloudflared` process.
...
- A remotely-managed Cloudflare Tunnel already created in the client's Cloudflare dashboard
...
After this CLI is merged to `main`:
...
Now `cfm` is available globally on the machine while pointing at the local checkout.
...
The Cloudflare accounts and domains remain isolated. `cfm` only manages the local connector processes.
...
- A Cloudflare Tunnel token is a sensitive credential. Rotate/revoke it in Cloudflare when access should be removed.

## Scope of v0.1

This version deliberately does **not** use a high-privilege Cloudflare API token and does not create DNS/routes automatically.
